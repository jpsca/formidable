...
`error`         | An error code (or `None`) like `"required"` or `"invalid"`. Only populated after the form is validated (via `form.validate()` or `form.is_valid`).
`error_args`    | Optional extra details for the error (if there is one). Populated together with `error`.
